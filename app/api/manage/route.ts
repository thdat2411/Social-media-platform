...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!totalJobPosts) {
            return NextResponse.json({ error: "No job posts found" }, { status: 404 });
        }
        const totalPages = Math.ceil(totalJobPosts.length / limit);
        return NextResponse.json({ jobPosts, totalPages }, { status: 200 });

    } catch {
        return NextResponse.json({ error: "Error fetching job data" }, { status: 500 });
    }
}
...
export async function PUT(req: NextRequest) {
    try {
        ...
        if (action === "hide") {
            ...
            return NextResponse.json({ message: "Job post restored" }, { status: 200 });
        }

    } catch {
        return NextResponse.json({ error: "Error hiding job post" }, { status: 500 });
    }
}