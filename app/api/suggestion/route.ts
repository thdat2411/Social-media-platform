...
export async function POST(req: NextRequest) {
    try {
        const { title, company_name, location, workplaceType, jobType, level } = await req.json();

        if (!title || !company_name || !location || !workplaceType || !jobType || !level) {
            return NextResponse.json(
                { error: "All parameters (jobTitle, company, location, skills, experience) are required." },
                { status: 400 }
            );
        }

        const description = await fetchJobDescription(title, company_name, location, workplaceType, jobType, level);
